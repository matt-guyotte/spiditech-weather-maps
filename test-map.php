    ...
    <body>
        <section id = "main">
            ...
            <div id = "picture-box">
                ...
                <div class = "fas fa-step-backward fa-2x slider-button" id = "slider-back"> </div>
                  ...
                  <div class = "fas fa-step-forward fa-2x slider-button" id = "slider-forward"> </div>
                </div>
...
<?php
$request = wp_remote_get( "https://spidimaps.s3-us-east-2.amazonaws.com/{paramname}_validtimes.json" );
...
function javascriptCall($result, $result2) {
  ...
  console.log(responseBackSlash3);
        ...
        var sliderForward = document.getElementById('slider-forward');
        ...
        buttonSlow.addEventListener('click', () => {
            stopFunction();
            playTimeout = setInterval(playFunction, 2000);
        })
        ...
        for(var u = 0; u < regionArray.length; u++) {
          ...
          var node = document.createTextNode(regionArray[u].regionFull);
          aTag.appendChild(node);
          var aTagId = regionArray[u].regionMin + 'RegionButton';
          var aTagName = regionArray[u].regionFull;
          aTag.setAttribute('id', aTagId);
          aTag.setAttribute('name', aTagName);
        ...
        sliderBack.addEventListener('click', () => {
            ...
            for(var i = 0; i < pictureObject.length; i++) {
                if(testImg.src == pictureObject[i].src) {
                    if(i !== 0) {
                        var oneBefore = i - 1;
                        testImg.src = pictureObject[oneBefore].src;
                        currentDateText.innerText = pictureObject[oneBefore].date;
                        picSlider.value = oneBefore;
                        break;
                    }
                    if(i == 0) {
                        var startOver = pictureObject.length - 1;
                        testImg.src = pictureObject[startOver].src;
                        currentDateText.innerText = pictureObject[startOver].date;
                        picSlider.value = startOver;
                        break;
                    }
                }
            }
        ...
        sliderForward.addEventListener('click', () => {
            ...
            for(var i = 0; i < pictureObject.length; i++) {
                if(testImg.src == pictureObject[i].src) {
                    if(i !== pictureObject.length - 1) {
                        var oneAfter = i + 1;
                        testImg.src = pictureObject[oneAfter].src;
                        currentDateText.innerText = pictureObject[oneAfter].date;
                        picSlider.value = oneAfter;
                        break;
                    }
                    if(i == pictureObject.length - 1) {
                        var startOver = 0;
